...
short_desc(bedroom,"complete with dirty laundry and heaps of old homework, the sad remains of many hard fought battles waged over Dr. Sundberg's homework").
...
whereami :- write("You are located in the"),
here(X),
write(X).

look_around:- write('you can see the following locations...').
...
transfer_unchecked(X,Y):-
    ...
    write("You have moved the "), write(X),
    write(" to the "), write(Y).
...
make_unchecked(X):-
write("You have made "), write(X),
asserta(has(X)),
asserta(location(X,inventory)).

make(X):-
make_check_1(X),
has(Y),
make_check_2(Y),
rite("You have made "), write(X),
asserta(has(X)),
asserta(location(X,inventory)).
...
#check to make sure you are in the right location
make_check_1(X):-
here(Y),
make_recipe(Z,_,_),
location(Z,Y),fail.
...
#Check to make sure you have items for make_recipe
make_check_2(X):-
make_recipe(_,X,_),fail.
make_check_2(_).

inventory:-
write("The following is in your inventory "),
has(X),
write(X).
...
is_located(X,Y):-location(X,Y).

show_contents(X):-is_located(Y,X),name(Y,N),short_desc(Y,Desc),
write(N), write(": "), write(Desc), write('\n'),fail.
show_contents(_).

is_connected(X,Y):-door(X,Y).
is_connected(X,Y):-door(Y,X).

show_connections(X):-is_connected(X,Y),name(Y,N),short_desc(Y,Desc),
write(N), write(": "), write(Desc),write('\n'),fail.
show_connections(_).
...
name(X,N),
long_desc(X,Desc),
write(N), write(': '), write(Desc),
write('\n \n'),

write(N), write(' is connected to the following: \n'),
show_connections(X),

write('\n \n'), write(N),
write(' is contains to the following: \n'),
show_contents(X).
